...
if (GetPlatformID() != PLATFORM_ID_ANDROID)
{
	...
	if (!GetFont(FONT_SMALL)->Load("interface/font_mono.rtfont")) return false;
	if (!GetFont(FONT_LARGE)->Load("interface/font_mono_big.rtfont")) return false;
...
void App::Kill()
{
	GetEntityRoot()->RemoveAllEntities();
...
//		PreloadKeyboard(); //only applicable to iOS, makes the keyboard pop up faster
	}
}
...
void App::Draw()
{
   ...
   //GetFont(FONT_SMALL)->DrawScaled(0,200, "Default - `2Green`` - default",1+SinPulseByMS(3000), MAKE_RGBA(255,255,255,50));
	...
	GetIrrlichtManager()->Render2D(); //give irrlicht manager a chance to draw its debug stuff
	BaseApp::Draw();
	PrepareForGL();
}
