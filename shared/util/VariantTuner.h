 ...
 * This class might be helpful for example in fine tuning some physics related constant. Pass some \c Variant that you are trying to find a suitable value for to this class. Then adjust the value incrementally during run time with the mouse wheel and see the effect of different values immediately on the screen. Off course the \c Variant that is manipulated needs to be used somewhere in the code base so that changing its value has a visible effect. This can be generally acchieved in two ways:
 *
 * 1. either pass a \c Variant to this class that is directly used by some other instance. For example:
