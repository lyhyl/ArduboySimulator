...
L_Particle::L_Particle()
{
	...
	vec = CL_Vec2f(0,0);
	accel_vec_1d_to_2d = CL_Vec2f(0,0);
	time_elapsed = 0;
}
...
void L_Particle::Setup(SurfaceAnim *surf, int life_t)
{
	vec = CL_Vec2f(0,0);
	accel_vec_1d_to_2d = CL_Vec2f(0,0);
	surface = surf;
	surf->SetBlendingMode(Surface::BLENDING_ADDITIVE);
	radian = 0;
	radian_rm = 0;
	life = life_t;
...
L_Particle::L_Particle(SurfaceAnim* surf, int life_t)
{
	motion_controller = NULL;
	delete_surface = false;
	delete_motion_controller = false;
...
{
	accel_vec_1d_to_2d = par.accel_vec_1d_to_2d;
	delete_surface = false;
	delete_motion_controller = false;
...
void L_Particle::set_velocity(const CL_Vec2f& v_t)
{
	vec = v_t;
	...
	{
		accel_vec_1d_to_2d = v_t;
		linear_set_magnitude(accel_vec_1d_to_2d, motion_controller->acceleration_1d);
	}
}
...
void L_Particle::set_blending_mode(Surface::eBlendingMode mode)
{
	
	surface->SetBlendingMode(mode);
}
...
CL_Vec2f L_Particle::get_velocity(void)
{
	return vec;
}
...
void L_Particle::sizing_process(void)
{
	if(remaining_life <= sizing_starting_time)
	{
		...
		{
			current_size = current_size + size_ratio * time_elapsed;
		}
	}
}
...
void L_Particle::rotating_process(void)
{
	if(remaining_life <= rotating_starting_time)
	{
		...
		if(rotating_type == 1)
		{
			current_radian = current_radian + radian*time_elapsed;
		...
		{
			float current_radian_t = linear_get_radian(vec);
...
void L_Particle::motion_process(void)
{
	if( motion_controller != NULL )
	{
		vec += motion_controller->gravity * time_elapsed;
	
		if( motion_controller->motion_type == L_1D_ACCELERATION )
			vec = vec + accel_vec_1d_to_2d * time_elapsed;

		else if( motion_controller->motion_type == L_2D_ACCELERATION )
			vec = vec + motion_controller->acceleration_2d * time_elapsed;

		else if( motion_controller->motion_type == L_POINT_ACCELERATION )
		{
			CL_Vec2f acc = motion_controller->acceleration_point_pos - CL_Vec2f(x_pos, y_pos);
			linear_set_magnitude(acc, motion_controller->acceleration_point_mag);

			vec = vec + acc * time_elapsed;
		}

		if( linear_get_sqr_magnitude(vec) > motion_controller->speed_limit_sqr )
			linear_set_magnitude(vec, motion_controller->speed_limit);
	}

	x_pos += vec.x*time_elapsed;
	y_pos += vec.y*time_elapsed;
