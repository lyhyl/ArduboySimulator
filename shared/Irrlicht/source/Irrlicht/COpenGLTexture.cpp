...
void COpenGLTexture::getImageData(IImage* image)
{
	...
	/*
	//SETH It's returning max texture size of 1?  huh?  Ignoring this stuff for now
		...
		case ECF_PVRTC:
			{
				...
				//rttex_header *pTexHeader = (rttex_header*)Image->lock();
				HasMipMaps = false; //we can never generate our own
				//Image->unlock();
			}
	...
	if (newTexture)
	{
...
#endif
		{
			...
			if (Image->getColorFormat() != ECF_PVRTC)
			{
				...
				if (Driver->testGLError())
					os::Printer::log("mipmaps", ELL_ERROR);
			}
		}
...
void* COpenGLTexture::lock(bool readOnly)
{
	...
	if (getColorFormat() == ECF_PVRTC)
	{
		assert(readOnly && "You can't write to a compressed texture");
		return Image->lock();
	}
