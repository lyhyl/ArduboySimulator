...
void CharComponent::UpLadder()
{
	...
	if (GetBuilding()->IsCloseToLadder(*m_pFloorID, m_pPos2d->x, vLadderPos))
	{
		...
		ZoomToPositionEntity(GetParent(), vLadderPos, climbSpeed);
		...
		AnimateStopEntityAndSetFrame(GetParent(), climbSpeed, 0, 0);
		GetMessageManager()->SetComponentVariable(this, climbSpeed, "floorID", (*m_pFloorID)+1);
		GetMessageManager()->SetComponentVariable(this, climbSpeed, "paused", uint32(0));
		return;
	} 
