...
SET RSYNC_OPTIONS=-az --chmod=Du=rwx,Dgo=rx,Fu=rw,Fgo=r --exclude='*.ncb' --exclude='*.lib' --exclude='*.exe' --exclude='*.dll' --exclude='*.pdb' --exclude='*.obj' --exclude='*.o' --exclude='*.ilk' --exclude='*.so' --exclude='.svn' --exclude='*.pch' --exclude='*.ipch'

:Need to know where your PI is, to ssh to (in my case, it's a local ip)
SET RASPBERRYIP=192.168.1.75
